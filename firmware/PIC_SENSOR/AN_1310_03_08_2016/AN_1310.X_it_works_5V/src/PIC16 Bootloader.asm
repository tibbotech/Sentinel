...
BootloadMode:
    ...
    banksel        OSCCON              ;bank1					    ; DELETE IT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!		
    movlw          b'11111011'         ;16kHz, PLLenable, INT_clock
   ...
   ; #ifdef  MOTHERBOARD 
    ...
    movwf   TRISC
    banksel	   LATC		       ;bank2
    bcf            LATC,3              ;turn on RTS RX_ENABLE
   ; #endif
   ...
   ; #endif
    ...
    banksel T1CON		    ; It is need to be there, because T1CON = bank 0, but it does not matter, because BRG16 no define
   ...
   ; goto LEDBLINK
    ...
    movlw   b'00010000'							   ; DELETE IT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    movwf   TRISA
    
   ; #ifdef  MOTHERBOARD 
    movlw   b'00100000'							   ; RTS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SETUP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ; #endif
    
    ;#ifdef  PROTOTYPE 
   ; movlw   b'00101000'							   ; RTS !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!SETUP!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ;#endif
...
#ifndef BRG16			    ;no BRG16 define
    
   ; goto LEDBLINK
    
    ;banksel OPTION_REG		    ; bank 1, do not need, because we already in bank 1
...
;goto LEDBLINK			    ; STX handshake is OK
...
SendHostByte:                       ; B0/B1 -> B1
  ...
  ; #ifdef  MOTHERBOARD 
    banksel RCSTA
    bcf     RCSTA, CREN             ; Stop receiving, so we dont receive what we send because rx enable is tied to ground
    banksel LATC
    bsf     LATC,3		    ;Set RS485 to TX Enable
  ; #endif
  ...
  ; bcf     RCSTA, CREN             ; Stop receiving, so we dont receive what we send because rx enable is tied to ground
  ; banksel LATA
  ; bsf     LATA,5		   ;Set RS485 to TX Enable
  ; bcf     LATA,2
 ;  #endif  
    ...
    clrwdt
    btfss TXSTA, TRMT
    bra WaitForTxToComplete
    
    
  ;  #ifdef MOTHERBOARD 
    banksel LATC
    bcf     LATC,3		 ;Set RS485 to RX Enable
    banksel RCSTA
    bsf     RCSTA, CREN         ; Renable receiving
  ;  #endif
    
  ;  #ifdef PROTOTYPE 
  ;  banksel LATA
   ;bcf     LATA,5		 ;Set RS485 to RX Enable
   ; bsf     LATA,2
   ; banksel RCSTA
   ; bsf     RCSTA, CREN         ; Renable receiving
  ;  #endif
